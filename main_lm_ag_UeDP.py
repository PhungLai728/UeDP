...
from utils_ag_ue import batchify_lm, get_batch_lm, repackage_hidden, get_data_user,get_data_ue_ent_ag
import pandas as pd
import copy
import pickle
...
parser.add_argument('--ent', type=str, default='loc',
                    help='type of entity')
parser.add_argument('--lr', type=int, default=100000,
...
if ent_type == 'gpe':
    list_epoch = [236,383, 537, 711, 907,1125,4513]# nu = 1000
elif ent_type == 'org':
    list_epoch = [73,137,201,270,347,434,1786] # nu = 500 
elif ent_type == 'loc':
    list_epoch = [195,324,456,603,771,958,3854] # nu = 500 
elif ent_type == 'all':
    list_epoch = [49,97,145,254,319,1327] # nu = 500 
elif ent_type == 'pii':
    list_epoch = [167,279,396,525,672,836,3373] #nu = 500 
...
name = 'ag_lr' + str(args.lr) + '_u' + str(user_per_epoch) + '_e' + ent_type + '_ep' + str(ent_percent) + '_clip' + str(S) + '_ns' + str(noise_scale) + '_emb' + str(args.emsize) + '_len' + str(seq_len) + '_' + str(load_wt103) + 'Wt103_30k_ueDP_gauss_seed' + str(args.seed) 
# name = name_resume + '_resume' 
dir_path = '../results/runs_lm_ueDP_AG_gauss/'
...
def evaluate(data_source, seq_len, user_, len_):
    ...
    for u in range(len(user_)):
        data, targets, len_sm = get_data_user(data_source, len_, u, user_, args, seq_len, evaluation=True)
        loss_tmp = 0
...
try:
    ...
    for epoch in range(nepochs + 1):# range(1, args.2epochs+1):
        ...
        for i in user_active:
            c += 1
            print(i)
            ue_data, ue_targets, ue_len = get_data_ue_ent_ag(train_data, train_len, i, num_user_train, args, seq_len,ent_active,dict_ent)
        ...
        if epoch % 5 == 0 or  epoch in list_epoch or epoch == nepochs:
            ...
            data_w = {'epoch': iter_, 'train ppl': ppl_train_all, 'test ppl': ppl_test_all, 
                'train loss': loss_train_all,'test loss': loss_test_all }  
