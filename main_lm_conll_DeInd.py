...
from utils_ag_ue import batchify_lm, get_batch_lm, repackage_hidden, get_data_user_conll, get_batch_ue_eval
import pandas as pd
import copy
import pickle
...
name = 'conll_lr' + str(args.lr) + '_user' + str(user_per_epoch) + '_clip' + str(S) + '_ns' + str(noise_scale) + '_emb' + str(args.emsize) + '_len' + str(seq_len) + '_' + str(load_wt103) + 'Wt103_DeInd_seed' + str(args.seed)  
dir_path = '../results/runs_lm_conll_DeInd/'
...
data4ue = np.load('../data/conll_2003/train_conll_de_seq' + str(seq_len) + '.npz',allow_pickle=True)
...
criterion = None
with open('../data/conll_2003/dictionary_conll_addmask', 'rb') as file:
...
try:
    ...
    for epoch in range(nepochs + 1): # range(1, args.2epochs+1):
        ...
        for i in user_active:
            ...
            c += 1
            print(i)
            user_data, user_targets, user_len = get_data_user_conll(train_data, train_len, i, num_user_train, args, seq_len)
        ...
        if epoch % 5 == 0  or epoch == nepochs: # or  epoch in list_epoch
            model_save(args.save)
