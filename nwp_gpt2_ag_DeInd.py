...
parser.add_argument('--rs', type=int, default=156,
                    help='resume epoch')
parser.add_argument('--ns', type=float, default=2.5,
                    help='noise scale ')
parser.add_argument('--de', type=str, default='mask',
                    help='de type ')
...
path_csv = 'results_nwp/DeInd/' + model_type + '_bs' + str(BATCH_SIZE)  + '_wt' + str(WARMUP_STEPS) + '_lr' + str(LEARNING_RATE) + '_max' + str(max_) + '_clip' + str(S) + \
    '_ns' + str(noise_scale) + '_seed' + str(args.seed)  + '_max0.19_' + args.de 
...
list_epoch = [1,32,63,94,125,156,200,250,300] #,187,218,234,249,280,311]
...
class AG_data(Dataset):
    def __init__(self, name, model_type, max_length, sheet, dataset_path = '../../data/'):
        super().__init__()
...
if args.de == 'mask':
    sheet = 'Masked Description'
else:
    sheet = 'Rem Description'

train_data = AG_data('train_preprocess_DeInd.csv', model_type, max_, sheet) 
# val_data = AG_data('valid_preprocess.csv', model_type, 1024) 
test_data = AG_data('test_preprocess.csv', model_type, max_, 'Description-ori') 
